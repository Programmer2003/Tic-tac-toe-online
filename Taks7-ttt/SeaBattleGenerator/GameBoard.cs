namespace SeaBattle
{
    class GameBoard
    {
        ...
        public int ShipCount()
        {
            ...
            {
                if (cell.isDeck && !cell.isBroken) count++;
            }
        ...
        public bool HaveShips()
        {
            foreach (var cell in Cells)
            {
                if (cell.isDeck && !cell.isBroken) return true;
            }

            return false;
        }

        public char[][] GetJsonField()
        {
            char[][] field = new char[boardSize][];
            for (int i = 0; i < boardSize; i++)
            {
                field[i] = new char[boardSize];
        ...
        public bool IsShip(int id)
        {
            if (id < 0 || id >= boardSize * boardSize) return false;

            int i = id / boardSize;
            int j = id - i * boardSize;
            return cells[i, j].isDeck;
        }

        public void Hit(int id)
        {
            if (id < 0 || id >= boardSize * boardSize) return; 
